...
export interface WallFace {
  ...
  halfT?: number
  // Вісь ПЕРПЕНДИКУЛЯРНОЇ стіни на кінці грані — і лише там, де цей кінець
  // справді ріг (а не розріз перегородкою). Ставиться тільки горизонтальній
  // грані: рогом «володіє» саме вона, і саме її оздоблення сцена доводить до
  // зовнішньої площини МАТЕРІАЛУ сусідньої стіни (тут, у чистій геометрії,
  // матеріалів ще не знають — закладено запас CORNER).
  cornerA?: number
  cornerB?: number
  // Докуди тягнеться темна ПІДКЛАДКА, якщо не до кінця грані. На розі елементи
  // загортаються аж до площини матеріалу сусіда — а підкладка мусить спинитись
  // раніше: інакше її торець лягає рівно в ту саму площину й на всю висоту рогу
  // йде смуга мерехтіння.
  backA?: number
  backB?: number
}
export const FACE_CORNER = CORNER
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        for (let k = 0; k + 1 < stops.length; k++) {
          ...
          out.push({
            ...
            a: fa,
            b: fb,
            cornerA: horizontal && k === 0 ? a : undefined,
            cornerB: horizontal && k + 2 === stops.length ? b : undefined,
          })
        }
      }
    }
