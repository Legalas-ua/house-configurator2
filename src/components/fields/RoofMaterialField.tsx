...
import { ROOF_MAT_KINDS, ROOF_SWATCHES } from '../../config/roofMaterial'
...
export default function RoofMaterialField() {
  ...
  const target: RoofMatSpec = part ? (perPart[part.id] ?? base) : base
  const patch = (p: Partial<RoofMatSpec>) => (part ? setPart(part.id, p, target) : setBase(p))
